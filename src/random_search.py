...
def random_search(model, sampler, sensitive_index, n_iter, chunk_size=None, return_details=False):
    ...
    # Generate all samples upfront, then batch-evaluate with two predict_proba calls
    # instead of 2*n_iter single-sample calls (FIX #28)
    ...
    # Build convergence curves: full chunks first, then remainder (FIX #30)
